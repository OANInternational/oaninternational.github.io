import { Event } from "@/components/event-preview/event-preview.component";

export const NEXT_EVENTS_ES: Event[] = [
  // {
  //   title: "Concierto Solidario OAN International",
  //   imageUrl: "/oan-web-053.jpg",
  //   description:
  //     "Concierto en directo con FullStop en la Sala Moby Dick de Madrid. Una noche de música donde cada acorde apoya los proyectos de OAN International en Nikki (Benín): acceso al agua, autonomía de mujeres rurales y lucha contra la malnutrición infantil. Música con impacto, energía y buen ambiente.",
  //   date: "18 de marzo de 2026, 19:30",
  //   price: "Consultar en Eventbrite",
  //   ticketUrl:
  //     "https://www.eventbrite.es/e/entradas-concierto-solidario-oan-international-1983885441761",
  // },
  // {
  //   title: "Las novedades de OAN International - 11º Aniversario",
  //   imageUrl: "/oan-web-043.jpg",
  //   description:
  //     "¡Encuentro anual! Conoce novedades de los proyectos, acércate más a la realidad de Nikki y disfruta con nosotros. Celebra con nosotros nuestro 11º aniversario en Talent Garden Madrid.",
  //   date: "29 de noviembre 2025, 18:00",
  //   price: "Donación",
  //   ticketUrl: "https://www.eventbrite.es/e/entradas-las-novedades-de-oan-international-1758692770419",
  // },
  // {
  //   title: "Magia en por Benin",
  //   imageUrl:
  //     "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQc7A1_vRvCOKP2NUrUb-6KQciSoM5AfI2xMQ&s",
  //   description:
  //     "Festival de magia benéfico en Madrid con magos de todas partes del mundo. Cada euro recaudado llegue íntegramente a Benín.",
  //   date: "14.03.2025 - 16.03.2025",
  //   price: "18€",
  // },
];
